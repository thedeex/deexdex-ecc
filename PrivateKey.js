...
class PrivateKey {
  ...
  get_shared_secret(public_key, legacy = false) {
    ...
    let KBP = _ecurve.Point.fromAffine(secp256k1, _bigi.default.fromBuffer(KB.slice(1, 33)), // x
    _bigi.default.fromBuffer(KB.slice(33, 65)) // y
    );

    let r = this.toBuffer();
    let P = KBP.multiply(_bigi.default.fromBuffer(r));
...
let toPublic = data => data == null ? data : data.Q ? data : _PublicKey.default.fromStringOrThrow(data);